...
An alternative approach is leftover typing, a technique that has been used to formulate intuitionistic linear logic \cite{Mackie} and to mechanise the linear \lambdacalc \cite{Allais2018a}.
...
\begin{enumerate}
  \item \textbf{Leftover typing for a resource-aware \picalc{}}. Our type system uses leftover typing to model the resource-aware \picalc{} (\autoref{leftover-typing}) and satisfies subject reduction.
    ...
    Thanks to leftover typing, we can now also define \emph{weakening} and \emph{strengthening} for the whole framework, not just the shared fragment.
...
The formalisation of the \picalc{} with leftover typing, from the syntax to the semantics and the type system, has been fully mechanised in Agda in about 1850 lines of code, and is publicly available in our \cite{Zalakain2020Agda}.
