...
\begin{nidefinition}[Types and typing contexts]
...
\begin{example}[Variable reference]
  Let us illustrate the use of variable reference relations with an example.
  \begin{lstlisting}[label=lst:channels,mathescape,numbers=left,caption=The underscore \_ introduces an anonymous declaration immediately followed by its definition.]
...
\begin{example}[Typing derivation]
  ...
  \begin{lstlisting}[label=lst:derivation,mathescape,numbers=left]
  $\func{p}$ : $\Process_1$
  $\func{p}$ = $\new{}$
...
\begin{remark}
  It is possible for the usage annotations of a variable $i$ within a process $P$ to be preserved as leftovers, yet for the variable $i$ to be mentioned by $P$: $P$ can choose to send $\lz$ usage annotations of $i$ over a channel.
\end{remark}

Swapping two variables preserves the well-typedness of a process.
Let $\func{sw}_i$ be a function that swaps variables $i$ and $\suc i$.
For simplicity, we overload $\func{sw}$ and use it to swap types in typing contexts and usage annotations in usage contexts. We use $\func{swap}_i \; P$ to swap variables $i$ and $\suc i$ within a process $P$, as introduced in structural congruence (\autoref{structural-congruence}).
\begin{nitheorem}[Swapping]
  \label{thm:swapping}
  Let $P$ be a well-typed process in $\types{\gamma}{\Gamma}{P}{\Xi}$.
  Then, $\types{\func{sw}_i \; \gamma}{\func{sw}_i \; \Gamma}{\func{swap}_i \; P}{\func{sw}_i \; \Xi}$.
\end{nitheorem}

\begin{remark}
  We limit ourselves to the swapping of adjacent variables $i$ and $\suc i$.
  Doing so is enough to prove subject reduction.
  More generally, any two variables can be swapped by a series of adjacent variable swappings.
\end{remark}
...
\begin{proof}[Proof (Sketch)]
  ...
  Here we will only consider the base cases for $\constr{struct}$ (and their symmetric variants under $\constr{sym}$), which are nontrivial. Full proof in \cite{Zalakain2020Agda}.
  ...
  \begin{itemize}
    ...
    \item
      ...
      We then use strengthening (\autoref{thm:strengthening}) to delete the unused variable from $P$, and compose the result with the typing derivation on $Q$ wrapped in $\constr{chan}$.
    ...
    \item
      Case $\constr{scope-comm}$: in one direction we use swapping (\autoref{thm:swapping}).
...
\end{nitheorem}
\begin{proof}[Proof (Sketch)]
  Use framing to derive $\contains{\gamma}{\Gamma}{j}{t}{m}{\Theta}$ and $\types{\gamma \comma t}{\Theta \comma m}{P}{\Xi \comma \lz}$ for some $\Theta$.
  Use these to apply \autoref{thm:subst-generalization}.
\end{proof}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item
    In the $\constr{struct}$ case, we apply subject congruence (\autoref{thm:subject-congruence}) and proceed inductively.
  \end{itemize}
\end{proof}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In this paper we provide a well-scoped syntax and a semantics for the \picalc{}, extrinsically define a type system on top of the syntax capable of handling linear, gradual and shared types, and show that reduction preserves the well-typedness of processes.
