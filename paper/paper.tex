...
\emph{In this paper we present the first full mechanisation in Agda of a \picalc{} with linear, graded and shared types, all under a common unified framework.}
...
\begin{example}[Typing derivation]
  ...
  In \autoref{lst:derivation} we illustrate a typing derivation using shared, graded and linear types.
  We use $\func{\omega}$ as a shortcut for $\one \comma \one$ in an algebra of shared types.

  \begin{lstlisting}[label=lst:derivation,mathescape,numbers=left,caption=Typing two processes that send parts of one channel over another.]
$\func{P}$ : $\Process_{\suc \suc \constr{0}}$
$\func{P}$ = $\comp{(\new{} \; (\send{\suc \; \constr{0}} {\constr{0}} \send{\constr{0}}{\suc \; \suc \; \constr{0}} \PO))}{\recv{\constr{0}}{\recv{\constr{0}}{}} \PO}$

$\func{\gamma}$ : $\PreCtx_{\suc \suc \constr{0}}$
$\func{\gamma}$ = $\constr{[]} \comma \unit \comma \channel{\channel{\unit}{\func{\omega}}}{\li}$

$\func{\vdash P}$ : $\forall \{n\} \to \types{\func{\gamma}}{\constr{[]} \comma \func{\omega} \comma (\suc n \comma \suc n)}{\func{P}}{\constr{[]} \comma \func{\omega} \comma (n \comma n)}$
$\func{\vdash P}$ = $\comp{(\new \; \unit \; \func{\omega} \; \one \; (\send{\suc \; \constr{0}}{\constr{0}} \; \send{\constr{0}}{\suc \; \suc \; \constr{0}} \PO))} {\recv{\constr{0}}{} \; \recv{\constr{0}}{} \PO}$

$\func{\vdash PP}$ : $\types{\constr{[]} \comma \unit}{\constr{[]} \comma \func{\omega}}{\new{} \comp{\func{P}}{\func{P}}}{\constr{[]} \comma \func{\omega}}$
$\func{\vdash PP}$ = $\new \; \channel{\unit}{\func{\omega}} \; \li \; (\suc \suc \constr{0}) \; (\comp{\vdash P}{\vdash P})$
  \end{lstlisting}

  Notice that the \emph{only} difference between the definition of the untyped process $\func{P}$ and its typing derivation are the extra type annotations that accompany scope restriction.
  This extra information requires a (limited) choice.
  To type $\func{P}$, we need a channel over which we can transmit another channel.
  We therefore create a new channel with usage annotations $\one \comma \one$ capable of transmitting data of type $\channel{\unit}{\func{\omega}}$, and declare (with $\li$) that we only transmit input capabilities over it (line 9).
  The left-hand process (line 3) is typed following its syntax (line 10).
  For the right-hand (lines 4-5) we need the channel that is going to be sent (lines 11-12).
