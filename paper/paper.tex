...
\begin{nidefinition}
  ...
  \end{aligned}
  \end{equation*}
  \caption{Algebra $\Algebra_C$ algebra on a partial commutative monoid.}
...
\begin{nidefinition}
  ...
  An indexed set of usage algebras is a nonempty type $\Idx$ of indices together with an interpretation $\Usage_{idx}$ of indices into types, and an interpretation of indices into usage algebras.
  $\Idx$ must be non-empty.
  Refer to \autoref{fig:indexed-multiplicities} for a formal definition.
  \begin{figure}[h]
    \begin{equation}
      \begin{split}
        &\Idx               &:{} &\Set \\
