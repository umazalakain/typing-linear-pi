...
\begin{note}
  ...
  We use the notation $\Usage^2$ to stand for a $\Usage \times \Usage$ tuple of input and output multiplicities, respectively.
  We use $\op{x}{y}{z}$ to stand for a monoidal operation on pairs of multiplicities -- where the algebraic laws are lifted elementwise.
...
\begin{nidefinition}
  ...
  An indexed set of usage algebras is a set of indices together with an interpretation of indices into types, and an interpretation of indices into usage algebras on that type.
  This indexed set must be non-empty.
...
Weakening states that adding new variables to the typing and input and output usage contexts preserves the well-typedness of a process.
The usage annotations of the new variables will be left untouched as leftovers.
...
\begin{nitheorem}
  ...
  Increment every referrence to a variable greater than or equal to $i$ in process $P$.
