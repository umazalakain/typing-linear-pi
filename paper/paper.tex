...
Its congruent equivalence closure \textsc{Equals} $=$ is defined1 in \autoref{app:struct}.
...
We identify two kinds of channels, $\constr{internal}$ and $\constr{external}\ i$, respectively standing for a channel which ... \todo{complete}
...
We use a $\func{dec}$ function to decrement the index of this channel every time we come out of a binder in rule $\constr{res}$. \todo{this seems more getting 'into' a binder than coming out...}
...
\begin{nidefinition}[Usage algebra]
  A \emph{usage algebra} is a ternary relation $\op{x}{y}{z}$, which is \emph{partial}, \emph{functional}, \emph{cancellative}, \emph{associative}, and \emph{commutative}.
...
\begin{description}
  \item [Linear]
    ...
    The partial monoid $\op{x}{y}{z}$ is defined exactly when $x \equiv y + z$. \todo{why isnt $\equiv$ here just =?}
...
\begin{nidefinition}[IDX set of usage algebra]
...
\begin{nidefinition}[Types and length-indexed typing contexts]
  \emph{Types} are defined in \autoref{fig:prectx} as either unit types (rule $\unit$), base types (rule $\base{n}$) or channel types (rule $\channel{t}{x}$).
  
  We keep track of types in a \emph{length-indexed typing context} $\PreCtx_n$, defined by the last row of rules in \autoref{fig:prectx}.
  A typing context is either empty $[]$ or building upon an existing typing context $\gamma$, it associates a type $t$ to an the next index in the context.
  \todo{check the last sentence}
  \begin{figure}[h]
  \begin{mathpar}
    \datatype
    { }
...
Notice that our notation of channel types is similar to Kobayashi's $[t]\mathbf{chan}_{(i^{y},o^{z})}$, where $y,z$ are the multiplicities of input and output \cite{K07}.

\begin{nidefinition}[Usage context]
  A \emph{usage context} in a context $\Ctx_{idxs}$ that is indexed by a length-indexed context of indices $\Idxs_n$, where we keep track of usage annotations (\autoref{fig:ctx}).

  A usage context is either empty $[]$ or building upon an existing context $\Gamma$, it associates a usage to the current index.
  \todo{check the last sentence}
