...
To do so, the standard approach is to use context splits\todo{cite}: typing rules for terms with multiple subterms use a side condition that specifies what resources to allocate to each of the subterms.
...
On top of that, context splits are usually binary, and thus typing rules with $n$ subterms require $n - 1$ context splits, which considerably clutters the type system.

An alternative approach to using context splits is using leftover typing \todo{cite}.
Leftover typing changes the shape of the typing judgments by including a second \emph{leftover} output context that contains the resources that were not used by the term.
As a result, typing rules \emph{thread the resources through} subterms without the need of context splits.
...
Below we present two alternative typing rules for process composition in the linear \picalc{}: the one on the left uses context splits, the one on the right leftover typing.
\begin{mathpar}
  \inferrule
...
\begin{itemize}
  ...
  \item We \textbf{generalise resource counting to a set of usage algebras that can be mixed within the same type system}.
    ...
    In particular, this allows for type systems combining linear, graded (exact number of times $n$) and shared (free to reuse and dicard) types under the same framework.
\end{itemize}
...
\todo{we have this in th intro already}
\begin{mathpar}
  \inferrule
  {\opctx{\Gamma}{\Delta}{\Xi} \\ \Delta \; \type{\vdash} \; P \\ \Xi \; \type{\vdash} \; Q}
  {\Gamma \; \type{\vdash} \; \comp{P}{Q}}
