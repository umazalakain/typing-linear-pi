...
\todo{maybe put both syntaxes in two columns to save space}
\begin{equation*}
  \begin{split}
...
\autoref{fig:syntax} shows the syntax of \picalc{} in Agda. We increase the \emph{depth} counter by 1 in channel restriction and input processes.
...
Machines can then manipulate references mechanically whilst using channel names to present \picalc{} processes back to humans.
...
Structural congruence $\eqeq$ is fully defined in \autoref{app:struct}.
\autoref{fig:struct-cong-base} lists the base cases.
...
The first three rules on $\constr{comp}$ state respectively, associativity, symmetry, and $\PO$ as the neutral element for parallel composition.
The last three rules on $\constr{scope}$ state respectively, garbage collection, scope extrusion and commutativity of restrictions.

In $\constr{scope-ext}$ the type $\Unused_i \; Q$ is an inductive proof that witnesses that the variable index $i$ does not appear neither in the inputs nor in the outputs of $Q$.
The function $lower_i \; Q \; uQ$ traverses $Q$ decrementing every index bigger than $i$.
In $\constr{scope-comm}$ function $swap_i \; P$ traverses $P$ (of type $\Process_{\suc \suc n}$) and swaps variable references $i$ and $\suc i$.
...
%In  \autoref{subject-reduction} we prove that if $P$ reduces to $Q$ and $P$ is well-typed, so is $Q$.

\todo{this paragraph below is not clear}
...
A typing context $\Gamma$ is a partial function from channel names to types.
A typing judgment is of the form $\Gamma \vdash P$, reading ``process $P$ is well typed under the typing context $\Gamma$''.
...
We characterise such an algebra in  \autoref{multiplicities}.
We define independent typing contexts and usage contexts in  \autoref{contexts}.
We provide a typing system with leftovers for a resource-aware \picalc{} in  \autoref{leftover-typing}.

\subsection{Multiplicities and Capabilities}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\label{multiplicities}

In the linear \picalc{} each channel has an input or an output \emph{capability}, and each capability has a given \emph{multiplicity} of 0 (exhausted) or 1 (available).
We generalise multiplicities by defining an algebra such that linear, gradual and shared types all satisfy these algebraic laws.
...
\begin{note}
  ...
  We use the notation $\Carrier^2$ to stand for a $\Carrier \times \Carrier$ tuple, and $\op{x}{y}{z}$ to stand for a monoidal operation on pairs of multiplicities -- where all properties are trivially lifted.
...
\todo{note I've changed the order of presentation from linear to shared, check if it breaks anything!}
Linear, gradual and shared types are all defined as an instance of the laws in \autoref{fig:multiplicities}.
We illustrate these types in typing derivations in \autoref{example-derivations}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\label{contexts}
\todo{is usage annotation the same as multiplicities? if yes, just use the former as too many notions}
In a resource-aware \picalc{}, usage annotations are transformed as a result of input and output; types however are preserved.
...
\begin{nidefinition}
  We define types (\autoref{fig:prectx}) as either unit types, base types or channel types.
...
\begin{nidefinition}
  ...
  An indexed set of usage algebras is a set of indices such that for every index there exists a carrier $C$ with a $\Quantifier_C$ algebra defined on it, as captured by \autoref{fig:indexed-multiplicities}.
...
\begin{nidefinition}
  A context of usages $\Ctx_{idxs}$ is indexed by a length-indexed context of indices $\Idxs_n$, as shown in \autoref{fig:ctx}.
  \begin{figure}[h]
  \begin{mathpar}
    \datatype
    {n : \N}
...
\begin{nidefinition}
  ...
  The \textsc{VarRef} relation presented in \autoref{fig:variable-references} is indexed by a typing context and an input and output usage context.
...
\begin{nidefinition}
  ...
  The typing relation \textsc{Types} presented in \autoref{fig:types} is indexed by a typing context, an input and an output usage context, and the process $P$ being typed.
...
decidable typechecking and type inference
limited to a linear type system
framing, weakening, substitution
soundness and completeness with respect to a sequent calculus for intuitionistic linear logic
