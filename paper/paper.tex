...
\subsubsection{Partial constructive specification}

Not well suited for typing with leftovers: you need to be able to inspect the
leftovers before producing them.

\subsubsection{Indexed addition}

Keep $\omega$ and naturals separated.

Extra level for these indices in the context.

Cannot saturate at 1: $1 = 1 \boxtimes 1$ is bad.

Needs cancelation.

Only closed processes get typechecked.

\subsubsection{Guarded subtraction}

Subtraction of $x - y$ only when $x \geq y$.

No cancellation: it is not the case that $n - n = 0$.
