...
\begin{abstract}
  ...
  We present an untyped syntax and semantics for the \picalc{}.
  We provide a resource-aware type system parametrised by a set of multiplicity algebras.
  This allows the user to mix \emph{linear}, \emph{graded} and \emph{shared} types.
...
The \picalc{} is a foundation for the design and implementation of programming languages for concurrency, such as Pict \cite{PierceT00} and (more recently) Go \footnote{https://golang.org}.
...
Enforcing the ``no duplicating'' and ``no discarding'' of resources via linearity allows for resource-aware programming and more efficient implementations \cite{Wadler90}.
...
%
Meanwhile, linearity acquired a different flavour with the rise of \emph{session types} \cite{H93,THK94,HVK98} (a formalism used in communication-centric programming), where linearity ensures that a channel is owned by exactly one communicating participant --- the channel itself is used multiple times, as per its session type.
...
The encoding has been used as a technique to implement session types in mainstream programming languages such as OCaml \cite{Padovani17} and Scala \cite{ScalasY16,ScalasDHY17}, and it allows to use the linear \picalc{} as an underlying theoretical and practical framework on top of which session types can be built.
...
The syntax uses de Bruijn indices for variables and is introduced in detail in \autoref{syntax}.
In short: $\new{} P$ creates a new channel and makes it available at index $0$ in the continuation $P$; $\comp{P}{Q}$ composes $P$ and $Q$ in parallel; $\recv{x}{}P$ receives along channel $x$ and makes the data available at index $0$ in the continuation $P$; and $\send{x}{y}{P}$ sends $y$ over channel $x$ and continues with $P$.
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[draw,circle]
    \node[draw,circle,fill opacity=0.3,text opacity=1,fill=orange] (send-x) at (0,2)   {$\func{send} \, x$};
    \node[draw,circle,fill opacity=0.3,text opacity=1,fill=orange] (send-y) at (2,2)   {$\func{send} \, y$};
    \node[draw,circle,fill opacity=0.3,text opacity=1,fill=green] (recv-z) at (4,2)   {$\func{recv} \, z$};
    \node[draw,circle,fill opacity=0.3,text opacity=1,fill=brown,align=center] (sync) at (2,0)   {$\func{sync}$\\$ \, x \, y \, z$};
    ...
    \draw[->] (sync) -- (recv-z);
  \end{tikzpicture}
  \caption{Four communicating example processes composed in parallel.}
  \label{fig:example-process}
\end{figure}

The example contains four processes that are composed in parallel: two of them send data over a channel; one receives data twice; the fourth sticks together and synchronizes the previous three processes (\autoref{fig:example-process}).
...
Thanks to linear and graded types, the processes below can have their interactions safely restricted: both $\func{send}$ processes consume $0$ input and $1$ output multiplicities; $\func{recv}$ consumes $2$ input and $0$ output multiplicities.
Dually, the process $\func{sync}$ consumes one input multiplicity on the channels shared with $\func{send}$, and two output multiplicities on the channel shared with $\func{recv}$.
We use these types to give a suitable typing derivation in \autoref{example-derivation} (\autoref{leftover-typing}).
...
\end{flalign*}
\begin{flalign*}
& \func{sync} \; : \; \Var_n && \to \Var_n \to \Var_n \to \Process_n & \\
& \func{sync} \; in0 \; in1 \; out \; &&= \\
& && \recv{in0}{} \\
& && \recv{(\suc \; in1)}{} \\
...
\end{flalign*}
\begin{flalign*}
& \func{example} && : \; \Process_{\constr{0}} & \\
& \func{example} && = \new{} ( \func{send} \; \constr{0} \\
& && \comp{}{ \new{} ( \func{send} \; \constr{0} } \\
& && \comp{}{ \new{} ( \func{recv} \; \constr{0} } \\
& && \comp{}{ \func{sync} \; (\suc \suc \constr{0}) \; (\suc \constr{0}) \; \constr{0}))) } \\
\end{flalign*}
...
The $\func{recv}$ process receives data along the channel with index $0$, thus that variable needs to be of type $\channel{t}{u}$.
After receiving twice, the process ends, so we should not be left with unused multiplicities: $u$ needs to be $\lz$.
...
The $\func{send}$ process sends data along the channel with index $0$, thus that variable needs to be of type $\channel{t}{u}$.
We instantiate the type of transmitted data $t$ to $\channel{\unit}{\func{\omega}}$, as that is something we can construct.
As by the type of the process $\func{recv}$, the transmitted multiplicities $u$ are $\lz$.
...
\begin{flalign*}
& \func{\vdash sync} && : \types{\gamma \comma \channel{t}{\lz} \comma \channel{t}{\lz} \comma \channel{t}{\lz}\\ & &&}{\Gamma \comma (\suc lx \comma rx) \comma (\suc ly \comma ry) \comma (lz \comma \suc \suc rz)\\ & &&}{\func{sync} \; (\suc \suc \constr{0}) \; (\suc \constr{0}) \; \constr{0}\\ & &&}{\Gamma \comma (lx \comma rx) \comma (ly \comma ry) \comma (lz \comma rz)} & \\
& \func{\vdash sync} && = \\
...
We can now put these processes together and omit specifying the types of the channels that they share: Agda can infer them from the types of the processes.

\begin{flalign*}
& \func{\vdash example} && : \; \types{\constr{[]}}{\constr{[]}}{\func{example}}{\constr{[]}} & \\
& \func{\vdash example} && = \new{} \; \_ \; \_ \; \_ \; ( \func{\vdash send} \\
& && \comp{}{ \new{} \; \_ \; \_ \; \_ } \; ( \func{\vdash send} \\
& && \comp{}{ \new{} \; \_ \; \_ \; \_ } \; ( \func{\vdash recv} \\
& && \comp{}{ \func{\vdash sync} }))) \\
\end{flalign*}
