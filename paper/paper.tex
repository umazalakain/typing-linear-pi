...
A courier system consists of three roles:
...
Our courier system is defined by four \picalc{} processes in parallel inhabiting the above three roles:
we have two \sender{} processes, $S_1$ and $S_2$, sending data over channels $x$ and $y$, respectively;
one \receiver{} process, $R$, which receives the data sent from each of the senders--hence receives twice;
...
In our framework, the syntax of \picalc{} processes adopts \emph{de Bruijn indices} \cite{deBruijn1972, Dybjer1994} for channel names and variables and is introduced in detail in \autoref{syntax}.
...
Both variable references $\Var_n$ and processes $\Process_n$ are indexed by the number $n$ of free variables they can refer to, for any natural number $n$.
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[draw,circle]
    ...
    \node[draw,circle,fill opacity=0.3,text opacity=1,fill=brown] (send-y) at (2,2)   {$\func{send} \, y$};
    \node[draw,circle,fill opacity=0.3,text opacity=1,fill=purple] (recv-z) at (4,2)   {$\func{recv} \, z$};
    \node[draw,circle,fill opacity=0.3,text opacity=1,fill=cyan,align=center] (sync) at (2,0)   {$\func{carry}$\\$ \, x \, y \, z$};

    \draw[->] (send-x) -- node[left] {$x$} (sync);
    \draw[->] (send-y) -- node[right] {$y$} (sync);
    \draw[->] (sync.35) -- node[right] {$z$} (recv-z.240);
    \draw[->] (sync) -- (recv-z);
  \end{tikzpicture}
...
Each of the roles \sender{}, \receiver{} and \courier{} is defined as a function respectively, $\func{send}, \func{recv}$ and $\func{carry}$ with signature $\Var_n \to \Process_n$.

The \sender{} role is defined below.
The \sender{} creates a new channel $\new{} \suc c$ denoting the data to be sent, and sends it over channel $c$, and then terminates, denoted by $\P0$.
\begin{flalign*}
  & \func{send} \; : \; \Var_n \to \Process_n & \\
  & \func{send} \; c = \new{} \send{\suc c}{\constr{0}} \PO
\end{flalign*}
Each \sender{} process $S_1$ and $S_2$ will  be an instantiation of $\func{send} \; c$.
In particular
$S_1$ will be defined as $\func{send} \; x$ and $S_2$ as $\func{send} \; y$, where data is sent over channels $x$ and $y$, respectively (see \autoref{fig:example-process}).
...
\end{flalign*}
The \receiver{} process $R$ will be instantiated as $\func{recv} \; z$, where $z$ is the channel used to receive data from the \courier{} (see \autoref{fig:example-process}).
...
\end{flalign*}
The \courier{} process $C$ will be instantiated as $\func{carry} \; x \; y \; z$, which sequentially receives on the two input channels $in0$ and $in1$, and then outputs the two pieces of received data on the output channel $out$. Here channels $x,y,z$ instantiate respectively $in0,in1,out$.
...
We use \textbf{shared types} to type the data that is being sent from the \sender{} processes to the \receiver{};
we use \textbf{linear types} for the usage annotations of [FILL IN, NOT CLEAR BEFORE];
and finally, we use \textbf{graded types} for the usage annotations of the communication channels $x$, $y$, and $z$: the \sender{} processes $\func{send} \; x$ and $\func{send} \; y$ each consume $1$ output multiplicity and the \receiver{} process $\func{recv} \; z$ consumes $2$ input multiplicities, as it receives twice on channel $z$.
Dually, the \courier{} process $\func{carry} \; x \; y \; z$ consumes one input multiplicity on $x$ and one input multiplicity on $y$, and two output multiplicities on $z$.
\end{example}
...
\item \textbf{Formalisation of the syntax and the semantics of the \picalc{}}:
 ...
 \end{itemize}
  ...
  \item \textbf{Fully mechanised formalisation and type safety results}:
    The formalisation of the \picalc{} with leftovers typing, from syntax to semantics and the type system, has been fully mechanised in Agda.
...
The syntax of the \picalc{} \cite{Sangio01} adopting channel names is given by the $\Raw$ grammar in \autoref{fig:syntax-names}.
...
Scope restriction $(\new{}x) \; P$ and input $\recv{x}{y} \; P$ are \emph{binders}, they are the only process constructs which introduce bound names --- $x$ and $y$ in $P$, respectively.
...
\begin{example}[$\func{fromRaw}$ and $\func{toRaw}$]
  \begin{figure}[t]
    ...
    \begin{equation*}
      ...
      \end{aligned}
    \end{equation*}
    \caption{From names to de Bruijn indices}
    \label{fig:conversion}
    \end{figure}
  We illustrate the conversion functions from names to de Bruijn indices $\func{fromRaw}$ and back $\func{toRaw}$ with three processes $P,Q,R$ given in \autoref{fig:conversion}.

  Process $P$ uses names $x,y,z$ and it is translated via the conversion function $\func{fromRaw}$ into process $Q$, which uses de Bruijn indices.
  ...
  [ADJUST SPACING OF THE EXAMPLE IN FIG 5]
\end{example}

In the following we present the main results that our conversion functions satisfy.

\begin{nilemma}
  Translating from de Bruijn indices to names via $\func{fromRaw}$ results in a well-scoped process.
\end{nilemma}

\begin{nilemma}
  Translating from de Bruijn indices to names via $\func{toRaw}$ results in a process with no shadowed variables.
\end{nilemma}

\begin{nilemma}
  Translating from de Bruijn indices to names and back via via $\func{fromRaw} \circ \func{toRaw}$ results in the same process modulo internal variable name hints.
\end{nilemma}
...
\begin{proof}[Proof (Sketch)]
  ...
  Complete details can be found in our mechanisation in Agda%
\begin{anonsuppress}
in \cite{Zalakain2020Agda}
\end{anonsuppress}
.
\end{proof}
...
We provide an example typing derivation for the processes defined in \autoref{example-process}.
For the sake of simplicity, we instantiate these processes with concrete variable references before typing them.

The $\func{recv}$ process receives data along the channel with index $0$, thus that variable needs to be of type $\channel{t}{u}$ for some $t$ and $u$.
...
The $\func{send}$ process sends data along the channel with index $0$, thus that variable needs to be of type $\channel{t}{u}$ for some $t$ and $u$.
We instantiate $t$ (the type of data that we transmit) to $\channel{\unit}{\func{\omega}}$, as that is a value we can easily construct.
As per the type of the process $\func{recv}$, the transmitted multiplicities $u$ are $\lz$.
We will transmit once, thus using a single output multiplicity, and leave the rest as leftovers.
...
Dually, the process doing the synchronisation expects input multiplicities for the channels shared with $\func{send}$ and output multiplicities for the channel shared with $\func{recv}$.
...
We can now put these processes together.
