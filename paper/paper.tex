...
\todo{check 'z' are replaced with 's' in formaliz*, mechaniz*; check modeling is modelling}
\todo{rename section and subsections in reference to \S (not sure why the commands above are not doing it so check later)}
...
For example ... \todo{complete the dots here, explain the rules above, in particular the 1+}
...
\begin{figure}[h]
  ...
  \begin{subfigure}[t]{.3\textwidth}
    \begin{equation*}
      ...
      \end{split}
    \end{equation*}
    \caption{Ill-scoped grammar with channel names}
...
\todo{It is not clear what ill vs well scoped means. Add a sentece saying: "By ill-scoped we mean... By well-scoped we mean... "}
...
Every time we go under a binder, the number of binders a variable might refer to increments by one, in rules $\constr{0}$ and $\suc n$. \todo{check this is right what i just said}
...
\section{Semantics}
\label{semantics}
Due to a well-scoped grammar in \autoref{fig:syntax}, the semantics of our language can now be defined on the totality of the syntax. We start by defining the structural congruence.
...
In \autoref{multiplicities} we characterise a usage algebra for our typing system. It defines how resources are \emph{split} in parallel composition and \emph{consumed} in input and output.
...
\todo{you cannot refer to IDX if it is in the appendix. Assume the reader will never look at the appendix. Maybe it is worth bringing back fig 12 or 13? Can only one be brought back?}
...
\todo{Explain what are Two and N. The whole sentence the carrier is a type Two is not clear...}
\begin{description}
  \item [Linear]
...
\end{description}
\todo{Added a last for example sentence to all. check}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
\subsection{Typing Contexts}
\label{contexts}
\todo{unify typing system and type system. only use one terminology in the paper.}
\begin{nidefinition}
...
\end{nidefinition}
\todo{again, usage and idx occur here, but the formal defintion seems to be actually necessary. I'm thinking that both fig 12 and 13 are needed in the main text...}
...
\begin{nidefinition}
  ...
  The base type $\base{n}$ uses natural numbers as placeholders for types --- the host language can interpret the former into the latter. \todo{I don't get this... give an example for int, and just talk about pi, rather than a host language.}
  %This is for convenience, so that there is less universe polymorphism polluting specifications.
  The type $\channel{t}{x}$ of a channel determines what type of data ${t}$ and what usage annotations ${x}$ are sent over that channel. This notation is similar to Kobayashi's $[t]\mathbf{chan}_{(i^{y},o^{z})}$, where $y,z$ are the multiplicities of input and output \cite{K07}.
  \todo{I don't really like the symbol between t and x. Why not a simple semicolon ; ?}
...
\begin{nidefinition}
  ...
  \end{figure}
\end{nidefinition}
\todo{Maybe fig 6 can go in appendix in order to make room for 12 and 13 to come back? This whole paragraph from def 6 below could be cut and brought to app? and only commented upon}
\begin{note}
...
\begin{itemize}
  ...
  \item
    ...
    Alternatively, such a proof would need to construct a witness $\opctx{\Gamma_m}{\Gamma_l}{\Gamma_r}$, where $\Gamma_l \vdash P$, $\Gamma_r \vdash Q$ and $\Gamma_m \vdash \comp{P}{Q}$.
  ...
  \item
    New theorems (\textbf{framing} (\autoref{framing}), \textbf{weakening} (\autoref{weakening}) and \textbf{strengthening} (\autoref{strengthening})) can suddenly be stated. \todo{explain suddenly}
\end{itemize}
  
We introduce two judgments: one on variable references (\autoref{def:varref}), one on processes (\autoref{def:types}).
Both are indexed by a typing context ($\gamma$), an input usage context ($\Gamma$), and an output usage context or leftovers ($\Delta$).
A typing context $\gamma$ keeps track of...  An input or output usage context $\Gamma,\Delta$ keeps track of...

\begin{nidefinition}
  \label{def:varref}

  The \textsc{VarRef} relation is presented in \autoref{fig:variable-references}.
  The judgment $\contains{\gamma}{\Gamma}{i}{t}{y}{\Delta}$ reads as "..." 
  The premise of \textsc{VarRef} means that...
  \todo{use parts from the commented sentences to complete the dots; refer to the greek letters.}
  \todo{Again, not sure I like the notation of this judgement...Instead of the square I would put $\triangleright$ If the relation is symmetric, then use a symmetric symbol, otherwise a symbol that conveys direction. Also, it seems as if the direction changes with that epsilon inverted.}
  % keeps track of the index, type, and usage annotation of a variable within a particular typing and usage context.
  % The leftover usage context is the result of subtracting the usage annotation from the input usage context.

  The base case $\constr{here}$ splits the usage annotation $x$ into $y$ and $z$, which are the leftovers.
  This splitting is as per the usage algebra defined on the index of the usage annotation in \autoref{}.
  $\constr{there}$...

  \todo{integrate the remark somewhere else, rather than in a remark?}
  \begin{remark}
...
\begin{example}
  ...
  \end{lstlisting}
  \autoref{lst:channels} asserts that the input $\channel{\unit}{\li}$ type at index $\constr{suc} \; \constr{0}$ is capable of $\li$, and that consuming such $\li$ results in $\lo$.
  The continuation means that...
  Finally, $= \constr{there} \; \constr{here}$ means that...
\end{example}
...
\begin{nidefinition}
  ...
  \todo{explain the rationale here as most people would expect a combination/splitting of typing context, which for us is not needed anymore. just add a sentence}
\end{nidefinition}
...
\begin{example}
  ...
  \end{lstlisting}
  \todo{not easy to follow. you need to expalin each bit of this derivation. when you comment you need to refer to what you are commenting on. either put things in bracket when commenting or refer to line number or whatnot}
\end{example}
...
\todo{Do not forward refer when it comes to results. Never refer to results in the appendix. Consider that, if the paper get publshed, it will get so without the appendix. It should be *completely* self contained}
\begin{nitheorem}
  \label{thm:framing}
...
\todo{Maybe all previous subsections could be put into one "Auxiliary Results"}
...
\todo{maybe only focus on the first case, internal and external and say the rest is by induction blah bla and details can be found in appenix/or github}
\begin{proof}
  \hfill{}\\
  \begin{itemize}
    \item
