...
\todo{lemmas in appendix are referred to from here}
\todo{shorten: we are slightly over the limit}
...
In a parallel track, J.-Y. Girard's development of linear logic \cite{Girard87} in the early '90s opened new avenues in computing science by introducing \emph{linear types} for (functional) programming languages \cite{Wadler90,Bernardy2018}.
...
Notice that this notation is similar to Kobayashi's $[t]\mathbf{chan}_{(i^{y},o^{z})}$, where $y,z$ are the multiplicities of input and output, respectively \cite{K07}.
...
two variables preserves the well-typedness of a process.
...
In our paper, we apply leftover typing from Allais \cite{Allais2018a} to the \picalc{} and focus on type checking. We give a more general usage algebra, leading to linear, graded and shared type systems.
Drawing from quantitative type theory (by McBride and Atkey \cite{McBride2016, Atkey2018}), in our work we are also able to talk about fully consumed resources.
As such, we can transmit over a channel all none multiplicities of a fully exhausted channel, namely a channel of multiplicity $\lz$. 

Recent years have seen an increase in effort to mechanise resource-aware concurrency calculi.
...
However, in his work context splits are extrinsic, and the types of used multiplicities erased, while we adopt a more general usage algebra.
Our framework integrates three different type systems and their combination in one and our linear \picalc{} uses leftover typing for the first time.
%
The work by Goto et al. \cite{Goto2016a} is, to the best of our knowledge, the first mechanisation of session types and their type safety proof (in Coq). 
...
%
Castro et al. provide tooling for locally-nameless representations of process calculi in Coq \cite{Castro2020} --- while de Bruijn indices are not as popular as dependent pattern matching, the latter is not as straightforward to handle.
...
\todo{related work above is so thoroughly presented. well done!
more to be added. Check what Cinzia di Giusto, Kirsten Peters, Antonio Ravara, Jorge Perez if they have any work published on mecahnisation (I know they are working on it).
Check Jesper, and Ivan Scagneto who has worked on mechanisiation of pi.
Check Brigite Pienta and her work on Beluga.
Next, check theories on linear pi or other languages and what is the world saying on linearity.
}
...
As a result, theorems like weakening and strengthening can now we stated for the linear \picalc{}.
Our work is fully mechanised in around 1500 lines of code in Agda \cite{Zalakain2020Agda}.

As future work, we intend to prove further properties of our type system, such as reduction preserves the balancing of channels.
...
Finally, we will use this our \picalc{} with leftovers as an underlying framework on top of which we can implement session types and other advanced type theories.
