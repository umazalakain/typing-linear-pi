...
As such, we can transmit over a channel $\lz$ multiplicities of a fully exhausted channel.

Orchard et al. introduce Granule \cite{Orchard}, a fully-fledged language with graded modal types, linear types, indexed types and polymorphism.
Modalities include exact usages, security levels and intervals.
Grade terms allow for grade polymorphism and their resource algebra is based on a pre-ordered semiring with monotonic multiplication and partial addition.
They provide bidirectional typing rules, and show the type safety of their semantics.

Recent years have seen an increase in the efforts to mechanise resource-aware concurrency calculi.
...
However, in his work context splits are extrinsic, and the types of used multiplicities erased.
...
%
Castro et al. provide tooling for locally-nameless representations of process calculi in Coq \cite{Castro2020} --- in Coq de Bruijn indices are not as popular as in Agda or Idris because dependent pattern matching is far from straightforward.
...
More generally, based on contextual type theory \cite{Pientkaa, Pientka}, LINCX \cite{Georges2017} extends the linear logical framework LLF \cite{Cervesato1996} by internalising the notion of bindings and contexts.
...
Further work on the mechanisation of the \picalc{} include \cite{Henry-Gerard1999, Honsell2001a, Bengtson2013, Despeyroux2000, Affeldt2008}, where the authors mechanise non-linear variations of the \picalc{}.
