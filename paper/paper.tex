...
\begin{example}[Variable reference]
  ...
  \begin{lstlisting}[label=lst:channels,mathescape,numbers=left,caption=Variable referece]
  ...
  \todo{use the caption to give a name to the example, not to comment much. Complete/change the incomplete name that I gave}
\end{example}
...
\begin{example}[Typing derivation]
  ...
  \begin{lstlisting}[label=lst:derivation,mathescape,numbers=left,caption= A typing derivation]
  $\func{p}$ : $\Process_1$
  $\func{p}$ = $\new{}$
