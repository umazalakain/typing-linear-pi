...
\begin{abstract}
  ...
  With this approach, context splits echo information that is already present within subterms, which already know about the resources they need.
  An alternative approach is to use leftover typing \cite{Mackie,Allais2018a}, where in addition to the usual (input) usage context, typing judgments have an additional output usage context: the leftovers.
  ...
  Our formalisation is mechanised in about 1800 lines of Agda.
...
\end{itemize}
\todo{mention we don't support recursion, why}
...
We have fully mechanised a proof of subject reduction, the details of which are found in \autoref{app:type-safety}
...
\begin{mathpar}
  ...
  \inferrule
  {eq : P \eq P'}
  {\constr{cong-comp} \; eq : \comp{P}{Q} \eq \comp{P'}{Q}}

  \inferrule
  {eq : P \eq P'}
  {\constr{cong-recv} \; eq : \recv{x}{}P \eq \recv{x}{}P'}

  \inferrule
  {eq : P \eq P'}
  {\constr{cong-send} \; eq : \send{x}{y}P \eq \send{x}{y}P'}
  ...
  \inferrule
  {eq : P \eq Q}
  {\constr{sym} \; eq : Q \eq P}

  \inferrule
  {eq_1 : P \eq Q \\ \; eq_2 : Q \eq R}
  {\constr{trans} \; eq_1 \; eq_2 : P \eq R}
\end{mathpar}
\end{nidefinition}
...
Leftover typing also allows us to prove a \emph{framing} theorem, which states that arbitrary usage annotations can be added to the input usage context, as long as they are added to the output usage context as well -- it is a way of understanding a typing derivation independently from its unused resources.
...
Moreover, leftover typing allows \emph{weakening} and \emph{strengthening} to acquire a more general form, in which variables can freely be added or removed from context regardless of their usage annotation -- as long as this usage annotation is added or removed from the leftover context as well.
