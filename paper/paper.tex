...
The \picalc{} is a formal language that abstracts over the details of concurrent processing and boils the interactions down to the sending and receiving of data through communication channels.
...
We define the syntax for the \picalc{} in \S \ref{syntax}.
We use dependent types to make this syntax well-scoped by construction: every unbound variable is accounted for in the type of the process using it.
\todo{de Bruijn}

In the \lambdacalc{} all computations are local and happen on syntactically contiguous terms; in the \picalc{} however computations need not to happen on syntactically contiguous terms.
One way to define a reduction relation modulo these syntactic minutiae is to define a structural congruence relation that acts as a quotient type.
A term $A$ can then be freely rewritten into a term $B$ as long as $A$ is structurally congruent to $B$.
We introduce this equivalence relation in \S \ref{structural-congruence}.
...
Two subprocesses can communicate anywhere in a process but under input and output, which must be respected to ensure the sequentiality in behavior.
We provide an operational semantics for the \picalc{} in \S \ref{operational-semantics}.
Every computational step is the result of communication; we take care to keep information on what channel the communication is happening on so that we can later use it to state the theorem for subject reduction (aka type preservation).

As with the \lambdacalc{}, the \picalc{} can be used to construct a wide variety of misbehaved processes.

The properties guaranteed by the linear picalc
...
\todo{stress what leftover typing is as a technique}
