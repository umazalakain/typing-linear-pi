...
\spnewtheorem{nidefinition}[definition]{Definition}{\bfseries}{}
...
\begin{abstract}
%  \todo{the only references here are to session types}
...
Recent years have seen an increase in the efforts to mechanise resource-aware process algebras \todo{cite}.
A resource-aware type system needs to keep track of what resources are used in which parts of the program.
To do so, the standard approach is to use context splits\todo{cite}: a typing rule for a term with multiple subterms will require an extra arguments that specifies what resources to allocate to each of the subterms.
The typing derivations for the subterms must then use the entirety of resources allocated to them.
A key observation here is that each subterm already \emph{knows} about the resources it needs, especially if these are linear.
\emph{Context splits do little more than repeat information that is already contained in the subterms.}
Moreover, it is impossible to provide typing derivations for the subterms until after the context splits have been provided.
On top of that, because in their usual presentation context splits are binary, a typing rule for a term with $n$ subterms requires $n - 1$ context splits, which adds to the clutter of the type system.

An alternative approach is to using context splits is using leftover typing \todo{cite}.
Leftover typing changes the form of the typing judgments themselves by including a second output \emph{leftover} context that contains the resources that were unused by the term.
As a result, typing rules can now \emph{thread the resources through} subterms without the need of context splits.
Each subterm uses the resources it needs, and leaves the rest for its siblings.
The first subterm in this chain of resources immediately knows about the resources it has available.

Leftover typing has been used to formulate intuitionistic linear logic \todo{cite} and to mechanise the linear \lambdacalc.
...
\begin{itemize}
  \item We use \textbf{leftover typing to type a resource-aware \picalc{} and prove subject reduction}.
    Our type system uses \emph{leftover typing} to model the resource-aware \picalc{} (\autoref{leftover-typing}).
    Leftover typing \textbf{makes context splits unnecessary}, allows for \emph{framing} (\autoref{thm:framing}) to be stated, and makes it possible to generalise \emph{weakening} (\autoref{thm:weakening}) and \emph{strengthening} (\autoref{thm:strengthening}) for the \picalc{}.
    
  \item We \textbf{generalise the resource counting to a set of usage algebras that can be mixed within the same type system}.
    The user can provide \emph{resource-aware} algebras that are then applied to the type system (\autoref{multiplicities}).
    ...
    This allows for type systems combining linear, graded (exact number of times $n$) and shared (free to reuse and dicard) types under the same framework.
\end{itemize}
...
We then introduce an interface for resource-aware algebras (\autoref{multiplicities}) and use it to parametrise a type system based on leftover typing (\autoref{leftover-typing}).
...
% \begin{enumerate}
%   \item We use \textbf{type-level {de Bruijn indices} \cite{deBruijn1972,Dybjer1994} to introduce a syntax for \picalc{} processes that is well-scoped by construction}:
%     Every free variable is accounted for in the type of the process that uses it (\autoref{syntax}).
%     \todo{Do Ciccone \& Padovani also use type level de Bruijn indices?}
% 
%   \item We \textbf{parametrise our type system over a set of resource-aware usage algebras}:
%     A \emph{usage algebra} is defined as a ternary relation that is \emph{partial}, \emph{deterministic}, \emph{cancellative}, \emph{associative} and \emph{commutative}.
...
% 
%   \item We \textbf{apply leftover typing to the \picalc{} and prove type preservation for the resulting type system}:
%     \todo{along the lines of Allais \cite{Allais2018a}}.
% \end{enumerate}
% 
% 
% 
...
In $\constr{scope-ext}$ the type $\Unused_i \; Q$ is an inductive proof asserting that the variable index $i$ does not appear neither in the inputs nor in the outputs of $Q$ (see \autoref{def:unused}).
...
\begin{nidefinition}
  ...
  \todo{should we have this as a figure instead?}
...
We lift the relation \textsc{StructCong} $\eqeq{}$ and close it under equivalence and congruence in \textsc{Equals} $\eq{}$ as shown in \autoref{fig:struct-cong1}.
...
\begin{equation*}
  ...
  \end{aligned}
\end{equation*}
\todo{introduce $\hookrightarrow$ notation?}
