...
Notably, it features \emph{channel mobility}: channels themselves are first order constructs, and can therefore be transmitted.
...
In the meantime, the rise of session types \cite{H93,THK94,HVK98} gave a different flavour to linearity in the \picalc{}.
Session types are a type formalism used in communication-centric programming.
In session types, linearity ensures that a channel is owned by exactly one communicating participant, even if the channel itself can be used multiple times, following the structure of its session type.
Linearity in session types is the key ingredient that guarantees type safety.

Recent work \cite{DardhaGS12,Dardha14,DardhaGS17} has pushed the linear \picalc{} into the spotlight again \cite{KPT96}, thanks to an encoding of session types into linear types.
...
This means that we can use the linear \picalc{} as an underlying theoretical and practical framework on top of which session types are built.

Considering the importance of the \picalc{} and of linearity in modeling concurrent and distributed systems, in this paper we focus our research on both and go beyond. \todo{}
We present the first full mechanisation in Agda of a \picalc{} with linear, gradual and shared types, all under the same unified framework.
...
Ultimately, we can use this work as a unified framework for type safe distributed modeling and programming with a variety of type systems, and built more advanced types, theories and languages on top.
While linearity is needed for type safety, gradual and shared types are needed for real-world systems.
...
\item \textbf{Formalisation of the syntax and the semantics of the \picalc{}}:
  ...
  \autoref{semantics} provides an operational semantics for the \picalc{}, prior to any typing.
  ...
  The relation is defined modulo structural congruence --- a relation that acts as a quotient type that gets rid of the unnecessary syntactic minutiae introduced by the syntax of the \picalc{}.
  ...
  Multiple algebras can be simultaneously used in a single type system --- usage contexts keep information about what algebra to use on which element.
  ...
  Leftover typing allows \emph{framing}, \emph{weakening} and \emph{strengthening} theorems to be stated.
  We use these to prove \emph{subject congruence}, and together with \emph{substitution}, to prove \emph{subject reduction}.  
...
After substitution, $\subst{P}{\suc j}{\constr{0}}$ is \emph{lowered} --- all variable references are decreased by one.
To do so, we apply \autoref{lm:subst-unused} in \autoref{app:lemmas} to obtain a proof $\Unused_{\constr{0}} \; (\subst{P}{\suc j}{\constr{0}})$.
...
\begin{nidefinition}
  ...
  We keep track of types in a length-indexed $\PreCtx_n$.
