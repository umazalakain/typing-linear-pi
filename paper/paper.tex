...
We now discuss the type safety of our type system, starting with auxiliary results.
...
In this section we move towards subject reduction.
We start with two important results: subject congruence (\autoref{thm:subject-congruence}) and substitution (\autoref{thm:subst-generalization} and \autoref{thm:substitution}).
...
\begin{proof}[Proof (Sketch)]
  By induction on $\types{\gamma}{\Gamma_i}{P}{\Psi_i}$. Full details are found in \autoref{app:subst-generalization}.
\end{proof}

The above theorem means that the substituted variable $i$ is capable of $m$ in $\Gamma_i$, and capable of $n$ in $\Psi_i$.
...
In their paper, they extend session types with polymorphism and pattern matching on session types.
...
Castro et al. provide tooling for locally-nameless representations of process calculi in Coq \cite{Castro2020} --- where de Bruijn indices are not as popular as dependent patter matching is not as straightforward to handle.
As an example use-case, they use their tool to help automate proofs of subject reduction for a type system with session types.
