...
Using both an input and an output usage context has several advantages.
For one, the usage modifications effected by both input and output can be enclosed within a self-containing typing and usage judgment for variable references -- also with an output context.
But more crucially, we avoid requesting an explicit constructive proof of context splitting for parallel composition:
if $P$ is typed under $\Gamma_l$ and $Q$ under $\Gamma_r$, then to type $\comp{P}{Q}$ under $\Gamma_m$ we would need a proof $\opctx{\Gamma_m}{\Gamma_l}{\Gamma_r}$.
...
\begin{nitheorem}
  If $P \reduce{c} Q$ and $\types{\gamma}{\Gamma'}{P}{\Xi}$ where $\opctx{\Gamma'}{\ell_{\#} @ c}{\Gamma}$,
  then $\types{\gamma}{\Gamma}{Q}{\Xi}$
\end{nitheorem}
