...
The idea de Bruijn had \cite{} was to use a natural number $n$ (aka \emph{index}) to refer to the variable introduced $n$ binders ago --- and binders no longer introduce names.
\begin{example}
\hfill{}\\
...
\begin{note}
  ...
  We use the notation $\Carrier^2$ to stand for the type $\Carrier \times \Carrier$, and $\opsquared{x}{y}{z}$ to stand for a monoidal operation on pairs of multiplicities -- where all properties are trivially lifted.
\end{note}
...
\begin{nidefinition}
  ...
  Like processes, variable reference judgments (Figure \ref{fig:variable-references}) are indexed by a typing context and an input and output usage context.
  In addition to these, variable reference judgments are also indexed by their position in the contexts, their type, and their usage annotation.
  
  The base case $\constr{zero}$ asserts that $t$ is the most immediate type in typing context, and splits the most immediate usage annotation $x$ in the input context into the usage annotation $y$ the usage annotation $z$ in the output context.
  ...
  \begin{mathpar}
    ...
    \inferrule
    {\opsquared{x}{y}{z}}
    {\constr{zero} : \contains{\gamma \comma t}{\Gamma \comma x}{\constr{zero}}{t}{y}{\Gamma \comma z}}
...
\begin{nidefinition}
  ...
  Our judgment on processes is of the form $\types{\gamma}{\Gamma}{P}{\Delta}$ (Figure \ref{fig:types}).
  This assertion combines a judgment on types ($\gamma$) and a judgment on usages ($\Gamma$ and $\Delta$).
  They could be kept as two separate judgments, but doing so would risk incoherence: if there is any choice to be made, the judgment on types could choose to go left while the judgment on usages chooses to go right.
  The constructions instroduced in this paper make the typing derivations completely syntax directed and thus this problem inexistent, but we nevertheless choose to keep both judgments as one for simplicity.
    ...
    \begin{figure}[h]
      ...
      \begin{subfigure}{.3\textwidth}
        \begin{alignat*}{2}
          ...
          \types{\gamma \comma t}{&\Psi \comma m}{P &&}{\Delta \comma \ell_{\o}} \\
      ...
      \begin{subfigure}{.5\textwidth}
        \begin{alignat*}{2}
          \types{\gamma \comma t}{& \Gamma \comma m}{P &&}{\Theta \comma \ell_{\o}} \\
          \contains{\gamma}{& \Theta}{j}{t}{m &&}{\Xi}
        \end{alignat*}
