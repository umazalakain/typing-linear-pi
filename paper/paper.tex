...
%
Meanwhile, linearity acquired a different flavour with the rise of \emph{session types} \cite{H93,THK94,HVK98}, a formalism used in communication-centric programming, where linearity ensures that a channel is owned by exactly one communicating participant --- the channel itself may be used multiple times, in particular by following its session type.
...
and finally a courier process $C$, which synchronises communication among the previous three process and
first receives data from $S_1$ and $S_2$ on its input channels, respectively $x$ and $y$ and then sends the received data to $R$ along its output channel $z$.
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[draw,circle]
    ...
    \node[draw,circle,fill opacity=0.3,text opacity=1,fill=blue,align=center] (sync) at (2,0)   {$\func{cour}$\\$ \, x \, y \, z$};
    ...
    \draw[->] (sync) -- (recv-z);
  \end{tikzpicture}
  \caption{The courier system: two senders $S_1$ and $S_2$, one receiver $R$ and one courier $C$.}
  \label{fig:example-process}
\end{figure}

The receiver process $R$ is defined as follows:
\begin{flalign*}
  & \func{recv} \; : \; \Var_n \to \Process_n & \\
  & \func{recv} \; c \; = \recv{c}{} (\recv{\suc c} \PO) \\
\end{flalign*}

The sender processes sender processes $S_1$ and $S_2$ are defined as follows:
\begin{flalign*}
  & \func{send} \; : \; \Var_n \to \Process_n & \\
  & \func{send} \; c = \new{} \send{\suc c}{\constr{0}} \PO \\
\end{flalign*}

The courier process $C$ is defined as follows:
\begin{flalign*}
  & \func{cour} \; : \; \Var_n && \to \Var_n \to \Var_n \to \Process_n & \\
  & \func{cour} \; in0 \; in1 \; out \; &&= \\
  & && \recv{in0}{} \\
  & && \recv{(\suc \; in1)}{} \\
...
\end{flalign*}
\begin{flalign*}
  & \func{courier} && : \; \Process_{\constr{0}} & \\
  & \func{courier} && = \new{} ( \func{send} \; \constr{0} \\
  & && \comp{}{ \new{} ( \func{send} \; \constr{0} } \\
  & && \comp{}{ \new{} ( \func{recv} \; \constr{0} } \\
  & && \comp{}{ \func{cour} \; (\suc \suc \constr{0}) \; (\suc \constr{0}) \; \constr{0}))) } \\
\end{flalign*}
[ALIGN FIG AND PROCESSES IN PARTICULAR send c and send x,y in THE FIGURE]
[COMMENT ON THE PROCESSES AND WHAT THEY DO]
[EXPLAIN HERE WHAT IS TYPED WITH LIN, GRADED AND SHARED]
Thanks to linear and graded types, the above processes can have their interactions safely restricted: both $\func{send}$ processes consume $0$ input and $1$ output multiplicities; $\func{recv}$ consumes $2$ input and $0$ output multiplicities.
Dually, the process $\func{cour}$ consumes one input multiplicity on the shared channels with $\func{send}$, respectively $x$ and $y$ in \autoref{fig:example-process} and two output multiplicities on the channel shared with $\func{recv}$, channel $z$ in \autoref{fig:example-process}.
