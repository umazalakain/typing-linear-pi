...
Names are cumbersome to mechanise: they offer no guarantee of being well-scoped, one has to deal with alpha-conversion, and inserting new variables into context entails proving that their names differ from all other names in context.
...
That is, terms at different \emph{depths} use different indices to refer to the same binding, and binders no longer introduce names.
\begin{example}
...
De Bruijn indices are easier to manipulate, but as humans we rather deal with names.
To that end, we provide the user with named processes, while using de Bruijn indexed processes internally.
We do so by defining translation functions in both directions and proving that they are inverses of each other up to $\alpha$-conversion.

\paragraph{From names to de Bruijn indices}
We keep the original binder names around as name hints when we translate into processes with de Bruijn names --- these names will serve as hints when we translate back.
The translation function $\func{fromRaw}$ works recursively, keeping a context $ctx : \Names_n$ that maps between the first $n$ indices and their names, and substituting named references within the process with their corresponding de Bruijn index.
...
The translation function $\func{toRaw}$ works recursively, keeping a context $ctx : \Names_n$ that maps between $n$ de Bruijn indices and names, and substituting indices within the process with their corresponding names.
As many widely-used languages do, we produce unique variable names but use the original variable names kept around throughout the de Bruijn representation as hints, to make the correspondence visually more straightforward.
For this, we use the naming scheme $<name-hint>\textasciicircum<n>$, where $<n>$ denotes that the name $<name-hint>$ has already been bound $n$ times before.
\begin{equation*}
  \begin{aligned}
...
\begin{nilemma}
  Translating from de Bruijn indices to names and back results in the same process modulo variable name hints.
\end{nilemma}
...
In $\constr{scope-comm}$ the function $\func{swap}_i \; P$ traverses $P$ (of type $\allowbreak \Process_{\suc \suc n}$) and swaps variable references $i$ and $\suc i$.
...
\begin{nidefinition}[Usage algebra]
  A \emph{usage algebra} is a ternary relation $\op{x}{y}{z}$, that is \emph{partial}, \emph{deterministic}, \emph{cancellative}, \emph{associative}, \emph{commutative}, and in which the leftovers can be \emph{computed}.
