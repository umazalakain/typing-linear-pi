...
\newcommand\iheart{%
    ...
    \llap{\makebox[\dimen0][c]{%
        \kern-0.000em%
        \raisebox{1.1ex}{\scalebox{0.30}{\ding{170}}}%
    }}%
    \endgroup
}
...
\begin{description}
  \item [Linear]
    ...
    All other properties are lifted elementwise.
...
\begin{note}
  ...
  We use the notation $\Usage_{idx}^2$ to stand for a $\Usage_{idx} \times \Usage_{idx}$ tuple of input and output multiplicities, respectively.
  We use $\opsquared{x}{y}{z}$ to stand for a monoidal operation on pairs of multiplicities -- where the algebraic laws are lifted elementwise.
...
\begin{nidefinition}
  ...
  \begin{mathpar}
    ...
    \inferrule
    {t : \Type \\ \stacked{idx : \Idx \\\\ x : \Usage_{idx}^2}}
    {\channel{t}{x} : \Type}
  \end{mathpar}
...
\begin{nidefinition}
  ...
  \begin{mathpar}
    ...
    \inferrule
        {\Gamma : \Ctx_{idxs} \\ x : \Usage_{idx} ^2}
...
\begin{nidefinition}
  ...
  \begin{remark}
    We use the functionality of the monoidal relation to aliviate the user from the proof burden $\opsquared{x}{y}{z}$.
  \end{remark}
...
\begin{example}
  The following example asserts that the input $\channel{\unit}{\li}$ type at index $\constr{suc} \; \constr{zero}$ is capable of $\li$, and that consuming such $\li$ results in $\lo$.
...
\begin{nidefinition}
  ...
  \begin{mathpar}
    ...
    \inferrule
    {t : \Type \\ x : \Usage_{idx}^2 \\ y : \Usage_{idx'} \\\\
  ...
  We use the leftovers $\Xi$ to type the continuation process, which is also provided with the received element --- of type $t$ and multiplicity $x$ at index $\constr{zero}$.
...
Adding a new variable to the context preserves the well-typedness of a process as long as the usage annotation of that variable is preserved unchanged as a leftover.

\begin{nitheorem}
  \label{thm:weakening}
  Let $P$ be a well-typed process $\types{\gamma}{\Gamma}{P}{\Xi}$.
  Insert an arbitrary type $t$ at position $i$ in the typing context, and an arbitrary usage annotation $x$ at position $i$ into both the input and output contexts.
  For simplicity, we overload $insert$ and use it to insert types in typing contexts and usage annotations in usage contexts.
  Increment every variable referrence greater than or equal to $i$ in process $P$.
  The resulting process is well-typed so that $\types{insert_i \; t \; \gamma}{insert_i \; x \; \Gamma}{lift_i \; P}{insert_i \; x \; \Xi}$.
\end{nitheorem}
\begin{proof}
  By induction on \textsc{Process} and \textsc{Var}.
\end{proof}
...
The proof that a variable is unused within a process must be syntactical and cannot rely on the preservation of its usage annotations as leftovers: a process can refer to a variable and still make no use of it --- by sending $\lz$ usage annotations over a channel.
...
\begin{nitheorem}
  ...
  Remove the elements at index $i$ from the typing and usage contexts and decrement every variable in $P$ greater than $i$.
  For simplicity, we overload $delete$ and use it to delete types from typing contexts and usage annotations from usage contexts.
  The result preserves the well-typedness of the process, so that $\types{delete_i \; \gamma}{delete_i \; \Gamma}{lower_i \; P}{delete_i \; \Xi}$.
\end{nitheorem}
\begin{proof}
  By induction on \textsc{Process} and \textsc{Var}.
\end{proof}
