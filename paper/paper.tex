...
The expression $\lambda g . (\lambda f . f g) g$ in the \lambdacalc{} would translate as $\lambda (\lambda 0 1) 0$.
...
As shown in Figure \ref{fig:process}, we increase the \emph{depth} counter every time we create a new channel or receive some input.
...
\begin{nitheorem}
  If $\types{\gamma}{\Gamma}{P}{\Xi}$ then $\types{insert_i \; t \; \gamma}{insert_i \; x \; \Gamma}{lift_i \; P}{insert_i \; x \; \Xi}$.
\end{nitheorem}

\subsection{Strengthening}

\begin{nitheorem}
  If $\types{\gamma}{\Gamma}{P}{\Xi}$ and $\Unused \; i \; P$, then $\types{delete_i \; \gamma}{delete_i \; \Gamma}{lower_i \; P}{delete_i \; \Xi}$.
\end{nitheorem}

\begin{remark}
  The proof that index $i$ is unused in $P$ must be syntactical and cannot rely on the usage annotation of index $i$ being preserved between $\Gamma$ and $\Xi$: process $P$ can send variable reference $i$ with multiplicity $\zero$ over a channel.
\end{remark}
...
\begin{proof}
  ...
  In the $\constr{scope-ext}$ case for a process $\new \comp{P}{Q}$, we will make use of Lemma $\ref{types-unused}$ to show that as the typing derivation on $P$ will preserved the multiplicity of the recently introduced variable unchanged.
  ...
  We will then use Lemma \ref{lift-lower} to show that lifting a lowered $P$ leaves $P$ unchanged.
  Finally, for the $\constr{scope-comm}$ rule, we will use swapping as shown in \S \ref{swapping}.
  In the reverse direction, we will use swapping and then show that swapping indices $i$ and $\suc i$ in process $P$ two times leaves $P$ unchanged.
\end{proof}
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}
    ...
    \node (psi-m) at (4,2) {$\Psi_m$};
    \node (psi-l) at (4,4) {$\Psi_l$};
    \node (psi-r) at (4,0) {$\Psi_r$};
    \draw[->]                (gamma) -- node[above,sloped] {$\Delta$} node[below] {$\Delta_i = \ell_{\o}$} (psi-m);
    \draw[->]                (gamma) -- node[above,sloped] {$\vdash P$}               (psi-l);
    \draw[->,densely dotted] (gamma) -- node[below,sloped] {$\vdash \subst{P}{j}{i}$} (psi-r);
    \draw[->]                (psi-m) -- node[right]        {$m@i$}                    (psi-l);
    \draw[->]                (psi-m) -- node[right]        {$m@j$}                    (psi-r);
  \end{tikzpicture}
  \caption{Diagrammatic representation of the generalized substitution theorem.}
  \label{fig:subst}
  \end{subfigure}
  ...
  \begin{tikzpicture}
    \node (gamma) at (0,2) {$\Gamma$};
    \node (xi-l)  at (0,4) {$\Xi_l$};
    \node (xi-r)  at (0,0) {$\Xi_r$};
    \node (psi-m) at (4,2) {$\Psi_m$};
    \node (psi-l) at (4,4) {$\Psi_l$};
    \node (psi-r) at (4,0) {$\Psi_r$};
    \draw[->]                (gamma) -- node[above,sloped] {$\Delta$} node[below] {$\Delta_i = \ell_{\o}$} (psi-m);
    \draw[->]                (gamma) -- node[left]         {$\ell_i@i$}               (xi-l);
    \draw[->,densely dotted] (gamma) -- node[left]         {$\ell_i@j$}               (xi-r);
    \draw[->]                (xi-l)  -- node[above]        {$\vdash P$}               (psi-l);
    \draw[->,densely dotted] (xi-r)  -- node[above]        {$\vdash \subst{P}{j}{i}$} (psi-r);
    \draw[->]                (psi-m) -- node[right]        {$m@i$}                    (psi-l);
    \draw[->]                (psi-m) -- node[right]        {$m@j$}                    (psi-r);
  \end{tikzpicture}
  \caption{
    Diagrammatic representation of the generalized substitution for process input.
    Thanks to the additional premise in Remark \ref{re:delta-zero}, we know that $m$ is at least capable of $\ell_i$, i.e. $\opsquared{m}{\delta}{\ell_i}$ for some $\delta$.
  }
  \label{fig:subst-input}
  \end{subfigure}
\end{figure}
