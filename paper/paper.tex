...
\begin{example}
  ...
  Let $\omega$ be a shortcut for $(\one \comma \one)$ in an algebra of shared types.
  ...
  We therefore create a new channel with usage annotations $(\one \comma \one)$ capable of transiting data of type $\channel{\unit}{\omega}$, and declare (with $\li$) that we will only transmit its input capability.
  ...
  $\func{\_}$ : $\types{\constr{[]} \comma \unit}{\constr{[]} \comma \omega}{\func{p}}{\epsilon}$
  $\func{\_}$ = $\constr{chan} \; \channel{\unit}{\omega} \; \li \; \one \; (\constr{comp}$
       $(\constr{recv} \; \constr{0} \; (\constr{recv} \; \constr{0} \; \constr{end}))$
       $(\constr{chan} \; \unit \; \omega \; \one \; (\constr{send} \; (\suc \; \constr{0}) \; \constr{0} \; (\constr{send} \; \constr{0} \; (\suc \; \suc \; \constr{0}) \; \constr{end})))
       )$
  \end{lstlisting}
\end{example}
