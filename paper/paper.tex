...
The complete structural congruence is given in \autoref{app:struct}.
...
The operational semantics of the \picalc{} is defined via a reduction relation in \autoref{fig:reduction}.
...
Reduction is closed under parallel composition (rule $\constr{par}$), scope restriction (rule $\constr{res}$) and structural congruence (rule $\constr{struct}$) 
--- notably, not under input nor output, otherwise the sequencing of actions would not be preserved \cite{Sangio01}.
...
Notice that the reduction relation in \autoref{fig:reduction} is effectful: it consumes the channel over which communication occurs.
If the channel is external to $P$ (it resides in its context) then the context in which $Q$ is typed must change.
...
We provide a typing judgment for a resource-aware \picalc{} in  \autoref{leftover-typing}.
