...
We present an example with four parallel processes: two processes that send data over a channel; one that receives data over a channel twice; and a fourth process that sticks the previous processes together and synchronizes them.

We start by defining the process doing the synchronization.
This process sequentially receives data on both input channels and then sends on the output channel both pieces of received data.
For simplicity, when we type this process we instantiate it to variable references and usage annotations.
We expect both input channels to be graded and capable of performing input once.
The output channel must be capable of performing output twice.
Notice that the well-typedness proof has no choice but to blindly follow the syntax.

\begin{flalign*}
& \func{\vdash sync} && : \types{\gamma \comma \channel{t}{\lz} \comma \channel{t}{\lz} \comma \channel{t}{\lz}\\ & &&}{\Gamma \comma (1 \comma 0) \comma (1 \comma 0) \comma (0 \comma 2)\\ & &&}{\func{sync} \; (\suc \suc \constr{0}) \; (\suc \constr{0}) \; \constr{0}\\ & &&}{\Gamma \comma (0 \comma 0) \comma (0 \comma 0) \comma (0 \comma 0)} & \\
& \func{\vdash sync} && = \\
...
Next, we define the processes doing the input and output.
This time too, we instantiate them to concrete variable references for typing.

\begin{flalign*}
& \func{\vdash recv} \; \types{\gamma \comma \channel{t}{\lz}}{\Gamma \comma (2 , 2)}{\func{recv} \; \constr{0}}{\Gamma \comma (0 \comma 2)} & \\
& \func{\vdash recv} \; = \; \recv{\constr{0}}{} \recv{(\suc \constr{0})}{} \PO \\
& \\
& \func{\vdash send} \; \types{\gamma \comma \channel{t}{\lz} \comma t}{\Gamma \comma (1 , 1) \comma \lz}{\func{send} \; (\suc \constr{0}) \; \constr{0}}{\Gamma \comma (1 \comma 0)\comma \lz} & \\
& \func{\vdash send} \; = \; \send{(\suc \constr{0})}{\constr{0}} \PO
\end{flalign*}

Finally, we put the four processes together in parallel.
It is here that the well-typedness proof must make a choice as to what types and usage annotations to instantiate new channels with.
In this case, these types and usage annotations were determined by the types of the previous subproofs, and we can thus omit most of them with $\_$ and have Agda find them.
We use $\func{\omega}$ as a shortcut for $\one \comma \one$ in an algebra of shared types.

\begin{flalign*}
& \func{\vdash example} && : \; \types{\constr{[]}}{\constr{[]}}{\func{example}}{\constr{[]}} & \\
& \func{\vdash example} && = \new{} \; \_ \; \_ \; \_ \; (( \new{} \; \_ \; \_ \; \_ \; \func{\vdash send}) \\
& && \comp{}{ \new{} \; \_ \; \_ \; \_ } \; (( \new{} \; \_ \; \_ \; \_ \; \func{\vdash send}) \\
& && \comp{}{ \new{} \; \channel{\unit}{\func{\omega}} \; \_ \; \_ } \; ( \func{\vdash recv} \\
