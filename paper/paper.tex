...
\todo{Theorem to Thm. Definiton to Def. when referencing. UMA: it's currently set by the lipics class, should we change it?}
...
\begin{example}[Typing derivation]
  ...
  We will now illustrate a typing derivation of the process $\func{p}$ using linear and shared types (Listing \autoref{lst:derivation}).
  ...
  We therefore create a new channel with usage annotations $(\one \comma \one)$ capable of transmitting data of type $\channel{\unit}{\func{\omega}}$, and declare (with $\li$) that we will only transmit its input capability (line 7).
...
\begin{nitheorem}[Strengthening]
  ...
  Then lowering every variable greater than or equal to $i$ in $P$ is well-typed in $\types{\func{del}_i \; \gamma}{\func{del}_i \; \Gamma}{\func{lower}_i \; P}{\func{del}_i \; \Xi}$.
\end{nitheorem}
...
For simplicity, we overload $\func{sw}$ and use it to swap types in a typing context --- we swap variable references in a process with $swap_i \; P$, as first used in structural congruence (\autoref{structural-congruence}).
...
Subject congruence theorem states that applying structural congruence rules to a well-typed process preserves its well-typedness.
...
\begin{proof}[Proof (Sketch)]
  The proof is induction on \textsc{Equals} $\eq{}$.
  ...
  We proceed by induction on \textsc{StructCong} $\equiv$:
  \begin{itemize}
    \item
