...
We do so by defining a specification for an algebra on $0,1,n$ and $\omega$ \emph{multiplicities} (how many times a channel can be used) and use it to apply for the first time leftover typing (following Allais \cite{Allais2018a}) to the \picalc{}.
...
Most of the work on mechanising resource-aware calculi is very recent.
...
Also in Agda.
Also de Bruijn.
We apply this to the \picalc{}.
We generalise the usage algebra.
Our type system is not bidirectional --- scope restriction needs usage annotations.
Adding bidirectionality would provide us with a decision procedure for type checking.

\cite{Thiemann2019}
intrinsicly typed
linear session types with recursion and subtyping.
also in Agda.
Also de Bruijn.
semantics as an intrinsically typed CEK machine
explicit context splits
MicroSession, minimal GV, lambda calculus with session types
contexts remove elements with multiplicity 0
keeps a global session context

\cite{Goto2016a}
first session-type paper with a mechanised proof of type safety
Session types
Coq
Extrinisically typed
Session type polymorphism
