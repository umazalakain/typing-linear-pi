...
This channel notation aligns with $[t]\ \mathbf{chan}_{(i^{y},o^{z})}$, where $y,z$ are the multiplicities of input and output, respectively \cite{K07}.
...
The \receiver{} defined by the $\func{recv}$ process receives data along the channel with index $0$, thus that variable needs to be of type $\channel{t}{u}$ for some $t$ and $u$.
...
The \sender{} defined by the $\func{send}$ process sends data along the channel with index $0$, thus that variable needs to be of type $\channel{t}{u}$ for some $t$ and $u$.
...
We can now put these processes together to type the courier system.
