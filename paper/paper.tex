...
We use \emph{leftover typing} \cite{Allais2018a} for our type system, an approach that, in addition to the usual typing context $\PreCtx_n$ and (input) usage context $\Ctx_{idxs}$, adds an extra {output usage context} to the typing rules.
...
\end{itemize}
  \todo{I still don't get the last point above, as weakening and strengthening were already possible to state in \picalc{} before. What do you mean with can be stated here?}
...
Both relations are indexed by a typing context $\gamma$, an input usage context $\Gamma$, and an output usage context $\Delta$ (containing the leftovers).

The typing judgement for variables $\contains{\gamma}{\Gamma}{i}{t}{y}{\Delta}$ asserts that ``index $i$ is of type $t$ in $\gamma$, and subtracting $y$ at position $i$ from $\Gamma$ results in $\Delta$''.
