...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[draw,circle]
    ...
    \node[draw,circle,fill opacity=0.3,text opacity=1,fill=darkgray] (recv-z) at (4,2)   {$\func{recv} \, z$};
    \node[draw,circle,fill opacity=0.3,text opacity=1,fill=blue,align=center] (sync) at (2,0)   {$\func{carry}$\\$ \, x \, y \, z$};
    ...
    \draw[->] (sync) -- (recv-z);
  \end{tikzpicture}
  \caption{The courier system: two \textcolor{orange}{senders} $S_1$ and $S_2$, one \textcolor{darkgray}{receiver} $R$ and one \textcolor{blue}{courier} $C$.}
  \label{fig:example-process}
\end{figure}

The receiver process $R$, defined below and instantiated as $\func{recv} \; z$, receives data twice on a channel $c$ and then terminates.
\begin{flalign*}
  & \func{recv} \; : \; \Var_n \to \Process_n & \\
  & \func{recv} \; c \; = \recv{c}{} (\recv{\suc c} \PO) \\
\end{flalign*}

The sender processes $S_1$ and $S_2$, defined below and instantiated as $\func{send} \; x$ and $\func{send} \; y$, create a new channel that acts as the data that is being sent, send it over the channel $c$, and then terminate.
\begin{flalign*}
  & \func{send} \; : \; \Var_n \to \Process_n & \\
  & \func{send} \; c = \new{} \send{\suc c}{\constr{0}} \PO \\
\end{flalign*}

The courier process $C$, defined below and instantiated as $\func{carry} \; x \; y \; z$, sequentially receives on the two input channels $in0$ and $in1$, and then outputs the two pieces of received data on the output channel $out$.
\begin{flalign*}
...
Finally, we fit these processes together and create three communication channels: the first shared between the sender $S_1$ and the courier $C$, the second between the sender $S_2$ and the courier $C$, and the third between the receiver $R$ and the courier $C$.
The result is the process defined below.
\begin{flalign*}
...
We continue this example and provide typing derivations for these processes in \autoref{leftover-typing}.
We use \textbf{shared types} for the type of data that is being sent over these channels, and \textbf{linear types} for its usage annotations.
We use \textbf{graded types} for the usage annotations of the communication channels $x$, $y$, and $z$ themselves: both processes $\func{send} \; x$ and $\func{send} \; y$ consume $1$ output multiplicity; $\func{recv} \; z$ consumes $2$ input multiplicities.
...
\begin{example}[Typing derivation, continues=exa:cont]
