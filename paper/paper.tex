...
\begin{example}[Variable reference]
  In $\func{egVar}$ we type a variable reference $\suc \constr{0}$ with type $\channel{\unit}{\li}$ and usage $\li$.
...
We provide a well-scoped syntax and a semantics for the \picalc{}, extrinsically define a type system on top of the syntax capable of handling linear, graded and shared types under the same unified framework and show that reduction preserves the well-typedness of processes.
...
As future work we intend to expand our framework to include infinite behaviour by adding process replication, which is challenging due to a combination of proving subject congruence and the need for type annotations in the syntax of processes.
%which can be added to a resource-algebra-agnostic type system like ours with the expected following rule:
%\begin{mathpar}
%  \inferrule
...
%To do so the resource consumption of a process has to be uniquely determined by e.g. adding type annotations to the syntax.
