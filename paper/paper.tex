...
Instead, we use de Bruijn indices \cite{}, where a natural number $n$ (aka \emph{index}) is used to refer to the variable introduced $n$ binders ago --- hence binders no longer introduce names.
\begin{example}
  \begin{equation*}
    \begin{split}
...
\autoref{fig:syntax} showcases a \picalc{} syntax that is well-scoped by construction thanks to type-level de Bruijn indices.
...
\begin{note}
  We lift the monoidal operation on usage annotations $\opsquared{x}{y}{z}$ to a monoidal operation $\opctx{\Gamma}{\Delta}{\Xi}$ on usage contexts with a common underlying context of indices.
