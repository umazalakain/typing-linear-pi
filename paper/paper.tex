...
\begin{example}
The \picalc{} term $(\textbf{new} \; x) (x ( y ) . y \langle \texttt{"Hello world!"} \rangle . \mathbf{0} \parallel (\textbf{new} \; y) (x \langle y \rangle . y ( z ) .\mathbf{0}))$ would translate to $(\textbf{new}) (0 () . 0 \langle \texttt{"Hello world"} \rangle . \mathbf{0} \parallel (\textbf{new}) (1 \langle 0 \rangle . 0 () . \mathbf{0}))$.
\end{example}
...
We make use \emph{leftover typing}, a technique consisting of adding an \emph{output usage context} to the usual \emph{input usage context}.
...
\item [Injectivity]
  We need to be able to \emph{subtract} multiplicities.
  To do so, we rely on $\op{x}{y}{z}$ uniquely determining $y$ for any $x$ and $z$.
...
\begin{nitheorem}
  ...
  Let $P$ be a process where the most recent variable has all its $m$ multiplities exhausted, such that $\types{\gamma , t}{\Gamma , m}{P}{\Psi , \ell_{\o}}$.
  ...
  As a result, the multiplicities at $\constr{zero}$ will be preserved and those at $j$ will be consumed instead, resulting in $\types{\gamma , t}{\Gamma , m}{\subst{P}{\suc j}{\constr{zero}}}{\Xi , m}$.
\end{nitheorem}
\begin{proof}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \begin{figure}[h]
      ...
      \begin{subfigure}{.5\textwidth}
        \begin{align*}
          \types{\gamma , t}{\Gamma , m}{lower_0 \; (\subst{P}{\suc j}{\constr{zero}})}{\Xi , m}
        \end{align*}
