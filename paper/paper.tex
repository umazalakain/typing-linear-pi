...
Together, these constraints give raise to a \emph{partial commutative monoid}.
In Figure \ref{fig:multiplicities} we present an algebra $\Quantifier_C$ on the carrier set $C$ in the form of a terniary relation such that it forms a partial commutative monoid.

\todo{SHOULD ALL THESE NOT BE DEFINITIONS?}
...
To allow for several multiplicity algebras to coexist on the same type system, we index the carriers of the algebras by an index $\Idx$ (see Figure \ref{fig:indexed-multiplicities}).
Each index $i$ has a corresponding carrier type $\field{CARRIER}_i$ such that $\field{CARRIER}_i$ satisfies the algebra $\Quantifier_{\field{CARRIER}_i}$.
We require the indexing type to be inhabited -- necessary to prove type preservation on $\new \PO_{\suc n} \eqeq \PO_n$.
\todo{lift op to $^2$}
...
Figure \ref{fig:prectx} represents base types $\base{n}$ using a natural number $n$ that can be reified into a type by the host language.
...
Types are then listed in a length-indexed \emph{pre-context} $\PreCtx_n$.
...
\S \ref{typing-judgments} introduces the typing rules for such a typing judgment.
...
\S \ref{variable-references} introduces the typing rules for such variable references.
...
\subsection{Variable references} \label{variable-references}
