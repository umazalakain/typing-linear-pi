...
module Scoped where
  ...
  data Scoped : ℕ → Set where
    𝟘     : Scoped n
    υ : Scoped (suc n) → ⦃ name : String ⦄ → Scoped n
    _∥_   : Scoped n → Scoped n → Scoped n
    _⦅⦆_ : Fin n → Scoped (suc n) → ⦃ name : String ⦄ → Scoped n
...
module Conversion where
  ...
  fresh' : Counter → Vec String n → String → String
  fresh' counter [] hint = hint
  fresh' counter (name ∷ ctx) hint = fresh' (inc counter name) ctx hint

  fresh : Vec String n → String → String
  fresh = fresh' init

  WellScoped : Vec String n → Raw → Set
  WellScoped ctx 𝟘 = ⊤
  ...
  WellScoped? : (ctx : Vec String n) (P : Raw) → Dec (WellScoped ctx P)
  ...
  NotShadowed : Vec String n → Raw → Set
  NotShadowed ctx 𝟘 = ⊤
  ...
  NotShadowed? : (ctx : Vec String n) (P : Raw) → Dec (NotShadowed ctx P)
  ...
  fromRaw' : (ctx : Vec String n) (P : Raw) → WellScoped ctx P → Scoped n
  fromRaw' ctx 𝟘 tt = 𝟘
  ...
  fromRaw : (ctx : Vec String n) (P : Raw) → ⦃ _ : True (WellScoped? ctx P) ⦄ → Scoped n
  fromRaw ctx P ⦃ p ⦄ = fromRaw' ctx P (toWitness p)

  toRaw : Vec String n → Scoped n → Raw
  toRaw ctx 𝟘 = 𝟘
  toRaw ctx (υ P ⦃ name ⦄) =
  ...
  map : ∀ {a} (B : Scoped n → Set a) (ctx : Vec String n) (P : Raw) → Set a
  map B ctx P with WellScoped? ctx P
  map B ctx P | yes wsP = B (fromRaw' ctx P wsP)
  map B ctx P | no _ = Lift _ ⊥

  map₂ : ∀ {a} (B : Scoped n → Scoped n → Set a) (ctx : Vec String n) (P Q : Raw) → Set a
