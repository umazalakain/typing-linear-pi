...
send-≥ℓₒ : {γ : PreCtx n} {Γ Δ : Ctx idxs}
         → γ ∝ Γ ⊢ i ⟨ j ⟩ P ⊠ Δ → ∃[ y ] (All.lookup i Γ ≔ y ∙ ℓₒ)
send-≥ℓₒ {Γ = Γ} (send {Δ = Δ} x _ _) = _ , ∙-comm (∋ℓₒ ℓₒ x)

recv-≥ℓᵢ : {γ : PreCtx n} {Γ Δ : Ctx idxs}
         → γ ∝ Γ ⊢ i ⦅⦆ P ⊠ Δ → ∃[ y ] (All.lookup i Γ ≔ y ∙ ℓᵢ)
recv-≥ℓᵢ (recv x _) = _ , ∙-comm (∋ℓₒ ℓᵢ x)

comm-≥ℓ# : {γ : PreCtx n} {Γ Δ : Ctx idxs} {c : Channel n}
      → P =[ c ]⇒ Q → γ ∝ Γ ⊢ P ⊠ Δ → c ≡ external i → ∃[ y ] (All.lookup i Γ ≔ ℓ# ∙ y)
comm-≥ℓ# {i = i} comm (comp (recv x ⊢P) ⊢Q) refl with ⊢-⊎ ⊢P | send-≥ℓₒ ⊢Q
comm-≥ℓ# {i = i} comm (comp (recv x ⊢P) ⊢Q) refl | (_ -, _) , (Ξ≔ , _) | _ , ≥-
  rewrite sym (∙-unique (∙-comm (proj₂ (proj₂ (∙-assoc (∙-comm (∋ℓₒ ℓᵢ x)) (proj₁ (proj₂ (∙-assoc⁻¹ (⊎-get i Ξ≔) ≥-))))))) (proj₂ ∙-join))
  = _ ,                  ∙-comm (proj₁ (proj₂ (∙-assoc (∙-comm (∋ℓₒ ℓᵢ x)) (proj₁ (proj₂ (∙-assoc⁻¹ (⊎-get i Ξ≔) ≥-))))))
...
align {i = i} {Γ = Γ} {Ψ = Ψ} x ⊢P x' Γ'⇒Γ | (Δ -, _) , (a , b) = ⊢-frame (a , b) (foo  , b) ⊢P
    ...
    bar = subst (λ ● → _ ≔ ● ∙ _) (sym (subst-idx ℓₒ)) (subst (λ ● → _ ≔ _ ∙ ●) (sym (subst-idx ℓᵢ)) (∙-comm (proj₂ (∙-join))))
...
subject-reduction Γ'⇒Γ (res_ {c = external zero} P→Q) (chan t m μ ⊢P) = chan t m _ (subject-reduction (Γ'⇒Γ , (proj₂ (comm-≥ℓ# P→Q ⊢P refl))) P→Q ⊢P)
