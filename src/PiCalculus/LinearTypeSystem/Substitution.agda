...
∋-subst : ∀ {γ : PreCtx n} {idxs : Idxs n} {Γ Γᵢ Γⱼ Δ Ψ Ψᵢ Ψⱼ Ξᵢ CONT : Ctx idxs} {tx : Type} {i j x : Fin n} {mx : Carrier idx' ²} {m n : Carrier idx ²}
        → γ ∝ Γᵢ [ i ]≔ t ∝ m ⊠ Γ
        → γ ∝ Γⱼ [ j ]≔ t ∝ m ⊠ Γ
        → γ ∝ Ψᵢ [ i ]≔ t ∝ n ⊠ Ψ
        → γ ∝ Ψⱼ [ j ]≔ t ∝ n ⊠ Ψ
        ...
          Σ[ Δ' ∈ Ctx idxs ]
          Σ[ m' ∈ Carrier idx ² ]
         (γ ∝ Γⱼ [ substFin j i x ]≔ tx ∝ mx ⊠ Ξⱼ
        × γ ∝ Ξᵢ [ i ]≔ t ∝ m' ⊠ Θ
        × γ ∝ Ξⱼ [ j ]≔ t ∝ m' ⊠ Θ
        × Θ ≔ Δ' ⊎ Ψ
        × All.lookup i Δ' ≡ ℓ∅)
∋-subst {i = i} {x = x} ∋i ∋j ∈i ∈j Γ≔Δ∙Ψ Δ∋iℓ∅ ∋x Ξᵢ≔CONT∙Ψᵢ with i Finₚ.≟ x
∋-subst {i = i} {x = x} ∋i ∋j ∈i ∈j Γ≔Δ∙Ψ Δ∋iℓ∅ ∋x Ξᵢ≔CONT∙Ψᵢ | yes refl with ∋-≡Idx ∋i | ∋-≡Idx ∋x
∋-subst {i = i} {x = x} ∋i ∋j ∈i ∈j Γ≔Δ∙Ψ Δ∋iℓ∅ ∋x Ξᵢ≔CONT∙Ψᵢ | yes refl | refl | refl =
  let
    typecheck = trans (∋-≡Type ∋j) (trans (sym (∋-≡Type ∋i)) (∋-≡Type ∋x))
    _ , Γᵢ≔ℓ∙Ξᵢ , ℓ≔ℓ = Only-⊎ (∋-Only ∋x)
    δ , _ , _ , _ , Ξᵢ≔δ∙Θ , Γ≔Δ₁∙Θ , Θ≔Δ₂∙Ψ , Δ₁[i]≡0 , Δ₂[i]≡0 = midpoint (∋-Only ∋i) Γ≔Δ∙Ψ Δ∋iℓ∅ Γᵢ≔ℓ∙Ξᵢ Ξᵢ≔CONT∙Ψᵢ (∋-Only ∈i)
    m≔ℓ∙δ = boil (∋-Only ∋i) (∋-Only ∋x) Ξᵢ≔δ∙Θ Γ≔Δ₁∙Θ Δ₁[i]≡0
  ...
  in _ , _ , _ , _
    , Only-∋ Γⱼ≔ℓ∙Γⱼ' typecheck
    , Only-∋ Ξᵢ≔δ∙Θ (∋-≡Type ∋i)
    , Only-∋ Ξⱼ≔δ∙Θ (∋-≡Type ∋j)
    , Θ≔Δ₂∙Ψ , Δ₂[i]≡0

∋-subst {i = i} {x = x} ∋i ∋j ∈i ∈j Γ≔Δ∙Ψ Δ∋iℓ∅ ∋x Ξᵢ≔CONT∙Ψᵢ | no i≢x =
  let
    _ , Ξᵢ≔m∙Θ , Γ≔ℓ∙Θ = diamond i≢x (∋-Only ∋x) (∋-Only ∋i)
    _ , Γⱼ≔ℓ∙Ξⱼ , Ξⱼ≔m∙Θ = reverse (∋-Only ∋j) Γ≔ℓ∙Θ
    _ , Γᵢ≔ℓCONT∙Ψᵢ , _ = ⊎-assoc⁻¹ (proj₂ (∋-⊎ ∋x)) Ξᵢ≔CONT∙Ψᵢ
    _ , Ξᵢ≔CONTn∙Ψ , _ = ⊎-assoc⁻¹ Ξᵢ≔CONT∙Ψᵢ (proj₂ (∋-⊎ ∈i))
    _ , Θ≔Δ'∙Ψ = outer-diamond i≢x (∋-Only ∋i) (∋-Only ∋x) Γ≔ℓ∙Θ Ξᵢ≔m∙Θ Γ≔Δ∙Ψ Ξᵢ≔CONTn∙Ψ
  in _ , _ , _ , _
   , Only-∋ Γⱼ≔ℓ∙Ξⱼ (∋-≡Type ∋x)
   , Only-∋ Ξᵢ≔m∙Θ (∋-≡Type ∋i)
   , Only-∋ Ξⱼ≔m∙Θ (∋-≡Type ∋j)
   , Θ≔Δ'∙Ψ , tail-ℓ∅ Γ≔Δ∙Ψ (proj₁ (proj₂ (Only-⊎ Γ≔ℓ∙Θ))) Θ≔Δ'∙Ψ Δ∋iℓ∅
...
⊢-subst-ih {i = i} ∋i ∋j ∈i ∈j Γ≔Δ∙Ψ Δ∋iℓ∅ end | Γi≔Δi∙Ψi | refl | refl | eq | qe
  rewrite Δ∋iℓ∅ | ∙²-unique Γi≔Δi∙Ψi ∙²-idˡ | ∙²-uniqueˡ qe eq | Only-uniqueʳ (∋-Only ∋i) (∋-Only ∈i) | Only-uniqueˡ (∋-Only ∋j) (∋-Only ∈j) = end
⊢-subst-ih ∋i ∋j ∈i ∈j Γ≔Δ∙Ψ Δ∋iℓ∅ (chan t m μ ⊢P) =
  chan t m μ (⊢-subst-ih (suc ∋i) (suc ∋j) (suc ∈i) (suc ∈j) (Γ≔Δ∙Ψ , ∙²-idʳ) Δ∋iℓ∅ ⊢P)
⊢-subst-ih {i = i} ∋i ∋j ∈i ∈j Γ≔Δ∙Ψ Δ∋iℓ∅ (recv {i = x} ∋x ⊢P) with ⊢-⊎ ⊢P
⊢-subst-ih ∋i ∋j ∈i ∈j Γ≔Δ∙Ψ Δ∋iℓ∅ (recv ∋x ⊢P) | (_ -, _) , (Ξᵢ≔δ⊢P∙Ψᵢ , _) with ∋-subst ∋i ∋j ∈i ∈j Γ≔Δ∙Ψ Δ∋iℓ∅ ∋x Ξᵢ≔δ⊢P∙Ψᵢ
⊢-subst-ih ∋i ∋j ∈i ∈j Γ≔Δ∙Ψ Δ∋iℓ∅ (recv ∋x ⊢P) | (_ -, _) , (Ξᵢ≔δ⊢P∙Ψᵢ , _) | _ , _ , _ , _ , ∋x' , ∋i' , ∋j' , Θ≔Δ'∙Ψ , Δ'∋iℓ∅ =
  recv ∋x' (⊢-subst-ih (suc ∋i') (suc ∋j') (suc ∈i) (suc ∈j) (Θ≔Δ'∙Ψ , ∙²-idʳ) Δ'∋iℓ∅ ⊢P)
⊢-subst-ih ∋i ∋j ∈i ∈j Γ≔Δ∙Ψ Δ∋iℓ∅ (send ∋x ∋y ⊢P) with ∋-subst ∋i ∋j ∈i ∈j Γ≔Δ∙Ψ Δ∋iℓ∅ ∋x (proj₁ (proj₂ (⊎-assoc⁻¹ (proj₂ (∋-⊎ ∋y)) (proj₂ (⊢-⊎ ⊢P)))))
⊢-subst-ih ∋i ∋j ∈i ∈j Γ≔Δ∙Ψ Δ∋iℓ∅ (send ∋x ∋y ⊢P) | _ , _ , _ , _ , ∋x' , ∋i' , ∋j' , Θ≔Δ'∙Ψ , Δ'∋iℓ∅ with ∋-subst ∋i' ∋j' ∈i ∈j Θ≔Δ'∙Ψ Δ'∋iℓ∅ ∋y (proj₂ (⊢-⊎ ⊢P))
⊢-subst-ih ∋i ∋j ∈i ∈j Γ≔Δ∙Ψ Δ∋iℓ∅ (send ∋x ∋y ⊢P) | _ , _ , _ , _ , ∋x' , ∋i' , ∋j' , Θ≔Δ'∙Ψ , Δ'∋iℓ∅ | _ , _ , _ , _ , ∋y' , ∋i'' , ∋j'' , Θ≔Δ''∙Ψ , Δ''∋iℓ∅
  = send ∋x' ∋y' (⊢-subst-ih ∋i'' ∋j'' ∈i ∈j Θ≔Δ''∙Ψ Δ''∋iℓ∅ ⊢P)
